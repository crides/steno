...
#define I2C_TRANSMIT(data) i2c_transmit((OLED_DISPLAY_ADDRESS << 1), &data[0], sizeof(data))
#define I2C_WRITE_REG(mode, data, size) i2c_writeReg((OLED_DISPLAY_ADDRESS << 1), mode, data, size, OLED_TIMEOUT)
...
bool oled_init(uint8_t rotation) {
    ...
    if (I2C_TRANSMIT(display_setup1)) {
    ...
    if (I2C_TRANSMIT(display_setup2)) {
...
void oled_render(void) {
    ...
    if (!HAS_FLAGS(oled_rotation, OLED_ROTATION_90)) {
        ...
        if (I2C_WRITE_REG(I2C_DATA, &oled_buffer[OLED_BLOCK_SIZE * update_start], OLED_BLOCK_SIZE)) {
        ...
        if (I2C_WRITE_REG(I2C_DATA, &temp_buffer[0], OLED_BLOCK_SIZE)) {
...
bool oled_scroll_right(void) {
    ...
    if (!oled_dirty && !oled_scrolling) {
        ...
        if (I2C_TRANSMIT(display_scroll_right)) {
...
bool oled_scroll_left(void) {
    ...
    if (!oled_dirty && !oled_scrolling) {
        ...
        if (I2C_TRANSMIT(display_scroll_left)) {
...
bool oled_scroll_off(void) {
    if (oled_scrolling) {
        ...
        if (I2C_TRANSMIT(display_scroll_off)) {
...
#if OLED_TIMEOUT > 0
    if (oled_active && timer_elapsed32(timer_read32()) > oled_timeout) {
        oled_off();
    }
#endif
